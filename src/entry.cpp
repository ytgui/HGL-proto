...
torch::Tensor _gcn_forward_cuda(
...
auto gcn_forward(const torch::Tensor &indptr,
                 const torch::Tensor &indices,
                 const torch::Tensor &features) {
    ...
    return _gcn_forward_cuda(indptr, indices, features);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("gcn_forward", &gcn_forward, "GCN forward (CUDA)");
