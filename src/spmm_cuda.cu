...
__global__ void spmm_cuda_kernel(
    ...
    const value_t *values, const index_t *indptr,
    const index_t *indices, const value_t *features,
...
torch::Tensor _spmm_cuda(const torch::Tensor &values,
    ...
    spmm_cuda_kernel<int32_t, float><<<n_nodes, min(32, n_features)>>>(
        n_nodes, n_features, values.data_ptr<float>(), indptr.data_ptr<int32_t>(),
        indices.data_ptr<int32_t>(), features.data_ptr<float>(), new_features.data_ptr<float>());
